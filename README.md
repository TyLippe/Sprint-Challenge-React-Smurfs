...
The first and biggest difference is that client-side takes place only on the users computer, while server-side takes place in a server on the web.  A way I look at it is the client side is kind of like the front end of everything while the server side is kind of the backend. 

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

Create, Read, Update, Delete will be used with axios methods such as get, post, put, and delete. 

- [ ] Mention three tools we can use to make AJAX requests.

I want to say we use fetch, catch and then. 
